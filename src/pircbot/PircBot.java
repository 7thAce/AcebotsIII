    ...
    private void updateUser(String channel, int userMode, String nick) {
        ...
        synchronized (_channels) {
            if (channel == null) {//@see #handleSpecial(String), this is all channels
                String[] channels = (String[]) _channels.keySet().toArray(new String[_channels.keySet().size()]);
                for (String s : channels) {
                    HashSet<User> userSet = (HashSet<User>) _channels.get(s.toLowerCase());
                    if (userSet != null) {
                        for (User u : userSet) {
                            if (u != null) {
                ...
                channel = channel.toLowerCase();
                HashSet<User> userSet = _channels.containsKey(channel) ? (HashSet<User>) _channels.get(channel) : new HashSet<User>();
                if (userSet == null) userSet = new HashSet<User>();//create a blank HashSet if it's still null somehow
                if (!userSet.isEmpty()) {
